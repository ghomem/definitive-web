...
from lib.utils import (generate_random_string, is_word, is_word_class, is_definition, is_see_also, is_avatar, validate_inputs,
                       hold_on, get_random_subtitle, get_autogenerated_definition)
...
# receive a POST request with the work to autodefine
# and return the definition in plain text
@app.route('/define', methods=['POST'])
def define():

    form_word = request.form['word']

    if is_word(form_word) is False:
        definition = ERROR_DEFINITION
    else:
        definition = get_autogenerated_definition(form_word)

    return render_template('textarea-div.html.j2', definition=definition, definition_max_length=DEFINITION_MAX_LENGTH)


# same as above but falling back to the default content
@app.route('/reset', methods=['POST'])
def reset():

    definition = DEFAULT_DEFINITION

    return render_template('input-fields-div.html.j2', word_max_length=WORD_MAX_LENGTH,
                           definition_max_length=DEFINITION_MAX_LENGTH, see_also_max_length=SEE_ALSO_MAX_LENGTH,
                           definition=definition, default_word=DEFAULT_WORD,
